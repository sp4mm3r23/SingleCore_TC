        ...
        for (uint8 i = INVENTORY_SLOT_ITEM_START; i < INVENTORY_SLOT_ITEM_END; i++)
        {
            if (Item* item = player->GetItemByPos(INVENTORY_SLOT_BAG_0, i))
            {
                ...
                if (proto)
                {
                    ...
                    if (proto->GetInventoryType() == invSlot && slot != SLOT_MAIN_HAND && slot != SLOT_OFF_HAND)
                        tempVector.push_back(item->GetEntry());
                    if ((proto->GetInventoryType() == invSlot || proto->GetInventoryType() == INVTYPE_2HWEAPON ||
                        proto->GetInventoryType() == INVTYPE_WEAPON) && slot == SLOT_MAIN_HAND)
                        tempVector.push_back(item->GetEntry());
                    if ((proto->GetInventoryType() == invSlot || proto->GetInventoryType() == INVTYPE_SHIELD ||
                        proto->GetInventoryType() == INVTYPE_HOLDABLE) && slot == SLOT_OFF_HAND)
                        tempVector.push_back(item->GetEntry());
                }
            }
        }
