    ...
    public:
        ...
        bool OnGossipHello(Player* player, Creature* creature) override
        {
            if (InstanceScript* instance = creature->GetInstanceScript())
            {
                ...
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_READY, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
        ...
        struct npc_neptulonAI : public ScriptedAI
        {
            ...
            void CompleteEncounter()
            {
                ...
                {
                    // Achievement
                    instance->DoUpdateCriteria(CRITERIA_TYPE_BE_SPELL_TARGET, SPELL_KILL_OZUMAT, 0, me); 
                        ...
                        {/* 
                            if (Player* player = i->GetSource())
                                if (Group* group = player->GetGroup())
                                    if (player->GetGuildId() && group->IsGuildGroup(player->GetGuildId(), true, true))
                                    {
                                        group->DoUpdateCriteria(CRITERIA_TYPE_BE_SPELL_TARGET, SPELL_KILL_OZUMAT, 0, 0, NULL, me);
                                        break;
                                    }
                        */
                        }
