            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    ...
                    case EVENT_UMBRAL_MENDING:
                        if (Creature* erudax = ObjectAccessor::GetCreature(*me, instance->GetGuidData(DATA_ERUDAX)))
...
class spell_erudax_shadow_gale: public SpellScriptLoader
{
    public:
        ...
        class spell_erudax_shadow_gale_SpellScript : public SpellScript
        {
            ...
            {
                BeforeHit += BeforeSpellHitFn(spell_erudax_shadow_gale_SpellScript::HandleBeforeHit);
            }
        };
