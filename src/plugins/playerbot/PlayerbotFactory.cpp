...
private:
    bool CanKeep(uint32 id)
    {
        ...
        if (proto->GetClass == ITEM_CLASS_MISC && (proto->SubClass == ITEM_SUBCLASS_JUNK_REAGENT || proto->SubClass == ITEM_SUBCLASS_JUNK))
            return true;
...
bool PlayerbotFactory::CanEquipItem(ItemTemplate const* proto, uint32 desiredQuality)
{
    ...
    if (level < 15)
        delta = urand(7, 15);
    else if (proto->GetClass == ITEM_CLASS_WEAPON || proto->SubClass == ITEM_SUBCLASS_ARMOR_SHIELD)
        delta = urand(2, 3);
...
void PlayerbotFactory::InitSecondEquipmentSet()
{
    ...
    do
    {
        ...
        for (ItemTemplateContainer::const_iterator i = itemTemplates->begin(); i != itemTemplates->end(); ++i)
        {
            ...
            else if (proto->GetClass == ITEM_CLASS_ARMOR && proto->SubClass == ITEM_SUBCLASS_ARMOR_SHIELD)
            {
...
void PlayerbotFactory::InitInventoryTrade()
{
    ...
    for (ItemTemplateContainer::const_iterator i = itemTemplateContainer->begin(); i != itemTemplateContainer->end(); ++i)
    {
        ...
        if (proto->GetClass != ITEM_CLASS_TRADE_GOODS || proto->Bonding != NO_BIND)
            continue;
...
void PlayerbotFactory::InitInventoryEquip()
{
    ...
    for (ItemTemplateContainer::const_iterator i = itemTemplateContainer->begin(); i != itemTemplateContainer->end(); ++i)
    {
        ...
        if (proto->GetClass != ITEM_CLASS_ARMOR && proto->GetClass != ITEM_CLASS_WEAPON || (proto->Bonding == BIND_WHEN_PICKED_UP ||
