...
void StatsAction::ListBagSlots(ostringstream &out)
{
    ...
    for (uint8 bag = INVENTORY_SLOT_BAG_START; bag < INVENTORY_SLOT_BAG_END; ++bag)
    {
        ...
        if (pBag)
        {
            ...
            if (pBagproto->GetClass == ITEM_CLASS_CONTAINER && pBagProto->SubClass == ITEM_SUBCLASS_CONTAINER)
            {
...
uint32 StatsAction::EstRepair(uint16 pos)
{
    ...
    if(LostDurability>0)
    {
        ...
        uint32 dmultiplier = dcost->multiplier[ItemSubClassToDurabilityMultiplierId(ditemproto->GetClass,ditemProto->SubClass)];
